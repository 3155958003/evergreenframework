...
public abstract class AbstractInjectHandler implements InjectHandler {
	 ...
	 * @return
	 */
	protected Object getBean(BeanFactory factory, Resource anno,
			Class<?> resourceType, String resourceName) {
		// 按name属性查找
		if (!"".equals(anno.name()) && anno.type() == Object.class) {
			return getByName(factory, anno.name());
		}
		// 按type属性查找
		if ("".equals(anno.name()) && anno.type() != Object.class) {
			return getByType(factory, anno.type(), resourceType);
		}
		// 按name和type查找
		if (!"".equals(anno.name()) && anno.type() != Object.class) {
			return getByNameType(factory, anno.name(), anno.type(),
					resourceType);
			// 默认匹配查找
		} else {
