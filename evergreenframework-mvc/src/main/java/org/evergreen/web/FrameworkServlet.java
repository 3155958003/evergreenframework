...
import org.evergreen.web.handler.WebMapHandler;
...
public abstract class FrameworkServlet extends HttpServlet {
	...
	public final static String SESSION = "org.evergreen.web.FrameWorkServlet.session";

	public final static String APPLICATION = "org.evergreen.web.FrameWorkServlet.application";

	/**
	 * 插件回调助手
	 */
	public final static String PLUGIN_HANDLER = "org.evergreen.web.pluginHandler";
	...
	public void init(ServletConfig config) throws ServletException {
		...
		// 初始化action回调处理器
		initInvokeHandler(config.getServletContext());
		// 初始化线程池
	...
	/**
	 * 初始化ActionDefinitions(所有Action的表述定义)
	 *
	 * @param config
	 */
	private void initActionDefinitions(ServletConfig config) {
		...
		List<String> classesName = initClassesName(scanPath);
		// 初始化所有Action的描述定义
	 ...
	 * 扫描获取所有的class类名
	 */
	private List<String> initClassesName(String scanPath) {
		return ScanUtil.scanPackage((scanPath == null) ? "" : scanPath);
	}

	/**
	 * 如果配置了容器插件,则初始化插件,那么action实例将从容器中获取 否则action实例将由mvc框架通过Action描述定义来构建实例
	 */
	private void initInvokeHandler(ServletContext servletContext) {
		String pluginName = servletContext.getInitParameter("plugin");
		// 目标Action回调处理器
		try {
			TargetActionHandler actionHandler = (pluginName != null) ? (TargetActionHandler) Class
					.forName(pluginName).newInstance() : new WebMapHandler();
			// 将回调处理器放入上下文
			servletContext.setAttribute(FrameworkServlet.PLUGIN_HANDLER,
					actionHandler);
		} catch (Exception e) {
			e.printStackTrace();
		}
